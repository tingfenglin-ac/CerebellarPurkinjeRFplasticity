...
% 2. Change the current folder to "Fig4 and Supp Fig4 PFstimulation_spatial pattern"
% 3. ctrl+enter to run each section

%% Section 1: Open files
% Select all file in the subfolders WT, L7SK2, and then CaMKII
% within the folder of "Fig4 and Supp Fig4 PFstimulation_spatial pattern"
...
y=data(i).yvalue';
x1=data(i).fPKS_pre'/max(data(i).fPKS_pre);
x2=data(i).fPKS_early'/max(data(i).fPKS_pre);
x3=data(i).fPKS_late'/max(data(i).fPKS_pre);
idx=find(x1<x2);
filly_pos=[x1 fliplr(x2)];filly_pos(idx)=x2(idx);
fill([x1 fliplr(x2)],[y fliplr(y)],[0.97 0.91 0.81],'EdgeColor','none')
fill(filly_pos,[y fliplr(y)],[182 208 226]/255,'EdgeColor','none')
plot(x1,y,'linewidth',linethink,'color',c(condlist(i)+1,:,1))
plot(x2,y,'linewidth',linethink,'color',c(condlist(i)+1,:,2))
% plot(x3,y,'linewidth',linethink,'color',c(condlist(i)+1,:,2))
...
% imagewd = getframe(gcf); 
% imwrite(imagewd.cdata, [FNlist{i} '_dF_' num2str(i) '.tiff' ]);
% close all
...
for i=5
    ...
    fill([x max(x) min(x)],[y2 0 0],[0.97 0.91 0.81],'EdgeColor','none')
